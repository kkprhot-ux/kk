...
class XunfeiASR(AudioTranscriber):
    """Xunfei RTAS WebSocket ASR provider.

    Verified against the official Java SDK (yijianguanzhu/iflytek-rtasr-websocket-client):
    - HMAC-SHA1 with APIKey (not APISecret)
    - URL param is lowercase appid (not app_id)
    - Algorithm is HMAC-SHA1 (not SHA256)
    - start frame must include transType: 'normal'
    - Server sends a 'started' string frame first, then 'result' dict frame
    - raise RuntimeError on error frames with the Xunfei code+description
"""
    ...
    async def _send_audio(self, ws, audio_chunk: bytes):
        ...
        # Xunfei protocol: first a 'started' string frame, then 'result' dict.
        # Loop until we get a dict (could be result or error).
        while True:
            raw = await ws.recv()
            try:
    ...
    def _extract_text(self, response: dict) -> str:
        try:
            ws_list = response.get("data", {}).get("result", {}).get("ws", [])
            return "".join(
                w.get("cw", [{}])[0].get("w", "")
                for w in ws_list
            )
